...
- **Afinidade por sala (K8s multi-réplica):** o SFU é in-memory por pod; o Redis propaga sinalização/presença, NÃO RTP. Todos os pares de uma sala têm de cair no MESMO pod → cliente envia `/ws?...&room=CODE` e o ingress faz `upstream-hash-by: $arg_room` (`deploy/k8s/40-ingress.yaml`). Sem isto: media num só sentido, admissão e partilha de ecrã falham. `/rtc` (presença) é fanned por Redis, não precisa de afinidade.
