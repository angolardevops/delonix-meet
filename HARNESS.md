...
- `net_probe.rs` — sondagem de rede pré-entrada, sem afinidade por sala: `GET /api/net-probe?bytes=` devolve enchimento (por omissão 256 KiB, tecto 4 MiB) com `Cache-Control: no-store`; `POST /api/net-probe` lê o corpo em bruto (`axum::extract::Request`, não o extractor `Bytes`, para o relógio arrancar antes da leitura) e devolve `{bytes, server_ms}` — o tempo que o cliente não consegue medir sozinho. 30 sondagens por conta por minuto (`net_probe_limiter`)
