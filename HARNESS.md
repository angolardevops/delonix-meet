...
**Deploy após alterações (REGRA):** depois de alterações merged, correr SEMPRE `make image-push` — gera imagens **versionadas** (tag `git describe`, ex.: `v1.0.0-16-g6d447e0`), faz `kind load` e **pina** a tag nos Deployments (`make pin`) com rollout status. Nunca confiar no `:latest` (imagem stale já causou "estilos perdidos" em stage). Ambos os hosts servem o **cluster kind** (VIP 172.30.0.200): `meet.delonix.local` E `cloud.delonix.local` (13/07/2026 — nginx local e systemd delonix-* foram REMOVIDOS; o ingress roteia os dois hosts; `publish-web.sh` é obsoleto). Volumes docker locais `delonix-meet_pgdata/redisdata` preservados como backup da BD antiga (migrada para o cluster; dumps em `backups/`).
