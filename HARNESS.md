...
- `net_probe.rs` — «qualidade prevista» da pré-entrada: `GET /api/net-probe?bytes=N` (bytes incomprimíveis, `no-store`, tecto 1 MiB) e `POST /api/net-probe` (`{bytes, server_ms}`, `413 net_probe.too_large`). Sessão; 60 sondagens por IP por minuto (`net_probe_limiter`, `429 net_probe.rate_limited`); não lê a base, não regista IP nem conta, a resposta só leva contagens
...
|---|---|---|
| PostgreSQL | 5435 | Dados principais (migrações 0001–0059; 0059 é número de trabalho desta linha) |
