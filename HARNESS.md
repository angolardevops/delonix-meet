...
- `media_probe.rs` — `ffprobe`/`ffmpeg`: duração (incl. webm do browser sem duração no cabeçalho), resolução, fps, codecs e miniatura, no fim da composição e no upload
- `recording_meta.rs` — leitor: publicação para a organização, miniatura, visualizações, participantes, transcrição com segmentos (descrição/etiquetas e comentários são de `recordings.rs`, paginados). Acesso SEMPRE por `recordings::access` (404 a quem não vê)
- `recording_chapters.rs` — edição (`PATCH`) e geração de capítulos (a lista, criação e remoção são de `recordings.rs`); automáticos (LLM local via `ai::generate`, a partir dos segmentos; varredura de 5 em 5 min); gerar de novo nunca apaga os manuais (G5)
- `recording_captions.rs` — legendas por língua em WebVTT (validação, rascunho/publicada, gerada da transcrição ou traduzida em segundo plano), servidas ao `<track>` (G5)
- `broadcast.rs` — emissão em directo para RTMP (ADR-0003): o browser compõe e codifica em H.264, o servidor REMULTIPLEXA (`-c:v copy`, `-c:a aac`). Multi-canal tipo StreamYard: **um `ffmpeg` POR destino** com supervisor (stderr drenado → `motivo`, `-progress` → `kbps`, backoff limitado, reentrada a meio do fluxo com o cabeçalho Matroska guardado; R126), estado por destino no WS (`{"tipo":"destinos",…}`), destinos em JSON na query ou `{id}` de um destino guardado (`stream_destinations.rs`, chave cifrada com `crypto.rs`/`SECRETS_KEY`), webhooks `stream.published`/`stream.ended`, tecto próprio `MAX_DESTINOS_POR_DIRECTO` (por emissão) distinto do `MAX_DIRECTOS` (por nó/sala). Recusa E2EE, codec não copiável, chave vazia, URL que não seja `rtmp(s)://` e acima de qualquer um dos dois tectos. Rota WS `/api/rooms/{code}/broadcast`; registo por sala
...
- `voice.rs` — PSTN: plano de controlo (DIDs, CDR, facturação, IVR por segredo partilhado em `/internal/v1/voice/ivr/*`), ramais (Fase 2: DID dedicado por ramal) e a ponte de media FreeSWITCH↔SFU (Abordagem B: mixer Opus, SRTP efémero); a media depende do operador SIP. O IVR é máquina-a-máquina e, no destino, sai da árvore pública para gRPC (ADR-0004 §4)
- `pstn_bridge.rs` — ponte de media entre o FreeSWITCH (RTP/Opus) e o SFU (SRTP): mixer de saída, SRTP de entrada e saída, lista branca de IP; activada por `voice::ivr_validate_pin` ao validar o PIN
...
- `sms_smpp.rs` — cliente SMPP 3.4 de saída (`bind_transmitter`/`submit_sm`/`unbind`) para Unitel/Movicel/Africell, `smpp://` ou `smpps://` (TLS sobre `AsyncRead + AsyncWrite`, backend `ring`); credenciais em `SMS_*_SMPP` (+ `SMS_<OPERADOR>_SMPP_CA` para o `smpps://`). Provado contra SMSC falso, **nunca contra um operador**; sem recibos
...
- `dlp.rs` — DLP (censura/redação de conteúdo sensível). Corre em TRÊS sítios, e a regra é «à entrada e outra vez no prompt» (R231): no que é difundido (`signaling.rs`: chat e legendas), no que é GRAVADO (`meetings::save_minutes`, `transcription::complete`, comentários de gravação) e no que sai para o LLM (`ai::caption_prompt`, `ai::minutes_prompt` — puros, testáveis sem Ollama)
...
- `ai.rs` — IA local via Ollama in-cluster: tradução de legendas em tempo real e resumo da ata. Fail-open sem `OLLAMA_URL` (o MoM cai para as regras do cliente); o texto das reuniões nunca sai para uma cloud externa O texto vai censurado para o modelo (R231): os prompts constroem-se em `caption_prompt`/`minutes_prompt`, que chamam o `dlp::censor`.
...
|---|---|---|
| PostgreSQL | 5435 | Dados principais (migrações 0001–0067) |
...
**HTTPS:** obrigatório para câmara/microfone fora de localhost. `make dev` já sobe um nginx standalone de dev (`make nginx-dev`, `deploy/nginx-dev.conf.template`, cert wildcard de `make certs`) em `https://meet.delonix.local`, que faz proxy para o Vite — usar esse em vez do `deploy/nginx-delonix.conf`, que é o de PRODUÇÃO (serve a SPA já compilada de `/var/www/delonix`, não o Vite em HMR).
