...
- `recorder.rs` — gravação server-side: RTP→IVF(VP8)+OGG(Opus), ffmpeg post-stop (VP9+Opus webm), E2EE via decrypt_e2ee(); a linha nasce em `processing` com progresso (`-progress`), cumpre `rooms.record_quality` (grelha na tela pedida, redução, só áudio) e a gravação automática (`auto_record_wanted`)
...
- `sms.rs` — gateway de SMS (ADR-0005): consola da org em `/api/orgs/{org_id}/sms/*` (só admin), superfície do agente USB em `/api/sms/agent/*` (token `dlxg_`, extractor `SmsGatewayAuth`), encaminhamento pelo plano de numeração angolano (prefixos **por confirmar**) e worker dos operadores que pára no drain. Envio a CONTACTO por `{user_id}` (número resolvido no servidor, membro activo, política `sms_send_policy`, opt-out da pessoa, limite por utilizador); `{to}` só de admin. Telefone por membro em `org_members` (`phone_source` odoo|manual). Entrega no máximo uma vez
...
- `sfu_e2e.rs` — testes ponta-a-ponta do SFU com `RTCPeerConnection`s reais no papel de browser (media a fluir nos dois sentidos + R13/glare + R156/troca de camada com simulcast real). Só compila em `#[cfg(test)]`
