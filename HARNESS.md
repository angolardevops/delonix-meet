...
- `meetings.rs` — calendário, conflitos, quarentena, MoM, transcrição, webhooks de meeting; opções de sessão (R184; migração 0055): `SessionOptions` (`format`, `waiting_room`, `auto_record`, `record_quality`) na reunião, passadas à sala no `start` e nos `PATCH` por `patch_session_options` (a mesma função para a v1); `PATCH /api/meetings/{meeting_id}` só opções e só o anfitrião; lista e `GET` com `invitee_count` e `external_source` (derivado de `meeting_external_refs`); RSVP `tentative`. Regras de forma em `domain::conferencing::session_options`. Testes: `server/tests/meeting_session_options.rs`
- `recordings.rs` — biblioteca de gravações, partilha read-only, upload (mede com `media_probe` e aceita `kind`), metadados no contrato da UI (R183; migrações 0052–0054): o item é o `RecordingLibraryItem` (ms, `kind`, `status`/`state`/`transcript_status` DERIVADOS, contagens, `caption_languages`, `description`/`tags`, `visibility`), `PATCH` de `filename`/`description`/`tags`, publicação `PUT/DELETE …/publication` (só `org`, 409 `recording.no_file` sem ficheiro), capítulos (`t_ms`, `source`, único por instante → 409; lista em array ≤100) e comentários (`t_ms`, `user_id`, `can_delete` = autor ou quem gere; apagar LÓGICO). UMA regra de acesso: `load_item` lê os factos numa só junção de pertença e `domain::content::recording::AccessFacts` decide (com `published_to_my_org`: membro ACTIVO de uma org do autor vê a publicada, não descarrega nem gere) — um membro arquivado (S3) perde tudo; `LIBRARY_VISIBLE_MINE/PUBLISHED` são `AccessFacts::listed_in` em SQL (teste `library_scopes_agree_with_access_facts`). `GET /api/recordings` sem `page_size`/`page_token` é uma lista (também com `q` e `scope=mine|published`); com eles, uma página. O ficheiro vai para `config.recordings_dir`. Testes: `server/tests/recordings_metadata.rs`
- `recording_meta.rs` — sub-recursos do leitor (R183): `GET …/thumbnail` (JPEG, 404 sem `has_thumbnail`), `POST …/views` (204, uma por pessoa por dia), `GET /api/recordings/{recording_id}/participants` e `GET /api/rooms/{room_code}/participants` (cursor `core::page`; sala só para quem participou, senão 404), `GET …/transcript` (estado derivado, segmentos limpos de `transcript_segments`, `error` só quando se desistiu). `load_segments` é o ponto de entrada da geração de legendas/capítulos (`recording_ai.rs`). Acesso sempre por `recordings::seen_item`
- `recording_captions.rs` — legendas WebVTT por língua (R183; tabela `recording_captions`): `GET …/captions` (≤50), `GET|PUT|PATCH|DELETE …/captions/{lang}`, `GET …/captions/{lang}/vtt` (`text/vtt`). Quem só vê lê só as PUBLICADAS (as outras dão 404); quem gere lê e escreve todas. VTT validado com a linha do erro, 2 MiB, 20 000 cues (`domain::content::caption`). A geração (`captions/generate`) está em `recording_ai.rs`; o VTT dá `409 recording.caption_not_ready` enquanto gera ou se falhou
- `recording_ai.rs` — capítulos e legendas gerados da transcrição pelo LLM local, ASSÍNCRONOS, só para quem gere a gravação (`managed_item`: 404/403 `recording.not_manager`). `POST …/chapters/generate` → `202` + `GET …/chapters/generation` (`idle|running|succeeded|failed` com `error_code`; tabela `recording_chapter_generations`, migração 0057): os capítulos entram como `source=auto` por `recordings::replace_auto_chapters` (o mesmo serviço e as mesmas regras dos manuais; um manual nunca é tocado) e **uma resposta sem capítulos utilizáveis é `failed ai.bad_response` e não marca `chapters_generated_at`** (B12 da linha antiga). `POST …/captions/generate` `{lang?, replace?}`: na língua da transcrição o VTT sai já dos segmentos (`201`/`200`, rascunho, `source=transcript`); noutra, tradução segmento a segmento em segundo plano (`202`, `generating` + `progress_pct`; um segmento sem tradução faz falhar a legenda inteira). Sem `replace` não substitui uma legenda que exista (`409 recording.caption_exists`). Modelo verificado ANTES de aceitar (`503 ai.*`), vaga por organização partilhada com o Estúdio (`429 ai.busy`); um trabalho sem sinal há mais de `OLLAMA_TIMEOUT_SECS`+60 s lê-se como `ai.interrupted` e pode repetir-se. Testes: `server/tests/ai_local.rs`
- `media_probe.rs` — `ffprobe`/`ffmpeg` sobre o ficheiro (duração em ms, resolução, fps, codecs, miniatura `<id>.jpg`), com tecto de 60 s por passo e passagem em cópia para o webm «ao vivo» do browser. `FFPROBE_BIN`/`FFMPEG_BIN`; sem eles (CI) os campos ficam NULL e `has_thumbnail=false` — nunca falha o upload nem o gravador; `COALESCE` não apaga a estimativa do gravador
- `recorder.rs` — gravação server-side: RTP→IVF(VP8)+OGG(Opus), ffmpeg post-stop (VP9+Opus webm), E2EE via decrypt_e2ee(); ao inserir grava `duration_ms` estimada (relógio de parede da sessão), `width`/`height` (grelha do xstack, ou cabeçalho IVF no remux) e `kind` (formato da sala), e depois mede o ficheiro com `media_probe` (que substitui o que medir); cumpre `rooms.record_quality` (R184: tela da grelha, redução de um só orador acima da qualidade, `audio` sem vídeo; NULL = composição de sempre) e decide a gravação automática (`auto_record_wanted`, chamada por `signaling` na entrada do anfitrião)
...
- `ai.rs` — IA local via Ollama in-cluster: tradução de legendas em tempo real e resumo da ata. Fail-open sem `OLLAMA_URL` (o MoM cai para as regras do cliente); o texto das reuniões nunca sai para uma cloud externa. `ai::Ollama` é o ÚNICO cliente do Ollama: `check_operator_url` + `state.outbound.operator()` (net_guard) e o erro verdadeiro com código estável (`domain::integration::llm::LlmFailure`: `ai.not_configured|url_rejected|unreachable|timeout|model_missing|upstream_error|bad_response`, sempre `503`, nunca o URL)
- `ai_assist.rs` — IA local do Estúdio por organização (membro activo; outra org `404`, decidido ANTES de ler o corpo): `GET /api/orgs/{org_id}/ai/status` (sempre `200`: `configured`, `reachable`, `model`, `model_installed`, `ready`, `reason`, `error`; tecto 3 s) e `POST /api/orgs/{org_id}/ai/suggestions` (`summary|publication|fillers`; nada se guarda; os termos de preenchimento só ficam se estiverem na transcrição como palavras inteiras; `400 ai.invalid_*`, `429 ai.busy` + `Retry-After: 10`, `503 ai.*`). Regras puras em `domain::content::ai_assist`. Config: `OLLAMA_MODEL_STUDIO` (omissão: o do resumo), `OLLAMA_TIMEOUT_SECS` (120, 5..=900, por chamada ao modelo), `AI_STUDIO_CONCURRENCY_PER_ORG` (1, 1..=8; `OrgSlots`, POR PROCESSO — com K réplicas o tecto é N×K)
...
- `meetings_v1.rs` — recurso `meetings` da API pública v1 (POST/PATCH/DELETE): cria REUNIÃO + sala com anfitrião humano (`host_email`) e convidados por email, idempotente por `external_ref`; as opções de sessão (R184) com as regras e o `PATCH` da BFF, e `422 meeting.auto_record_e2ee`. É o que a integração de calendário usa — `/api/v1/rooms` cria salas sem anfitrião nem convidados, e ninguém consegue ser admitido nelas
...
- `stream_destinations.rs` — destinos de emissão em directo guardados por organização (G1; migração 0042): CRUD de admin com o contrato novo (`201` + `Location`, `204`, paginação por cursor, `rotate-key` como método personalizado), chave RTMP cifrada com `core::secret_box` (contexto = id da linha) e devolvida só na criação e na rotação; `resolve_for_broadcast` decifra-a no servidor para o `ws_directo` (`destination_ids` + `org_id`, só admin), sem ela voltar ao browser. Regras de forma em `domain::content::stream_destination`
...
|---|---|---|
| PostgreSQL | 5435 | Dados principais (migrações 0001–0057) |
