...
--
-- Porto do Lote B (frontend/ui-template-rebuild) para cima da `main`: nasceu
-- 0051 na branch de origem; renumerada para 0053 (0052 é o topo da `main` a
-- 2026-09-19, `chat_retention_days`). As 5 partes da migração de origem
-- reunidas num só ficheiro — não há razão para as separar aqui, e uma só
-- migração evita a reconciliação de `_sqlx_migrations` entre lotes.
