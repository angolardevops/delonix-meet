...
async fn sms_policy_only_admin_reads_and_writes(db: sqlx::PgPool) {
    ...
    let (st, v) = app.get(&format!("/api/orgs/{}/sms/policy", a.org()), Some(&a.token)).await;
    assert_eq!(st, 200, "{v}");
    assert_eq!(v["send_policy"], "admins");

    // Membro sem papel de admin: 403.
    let (st, _) = app.get(&format!("/api/orgs/{}/sms/policy", a.org()), Some(&carla.token)).await;
    assert_eq!(st, 403);
    let (st, _) = app
        .put(
    ...
    let (_, v) = app.get(&format!("/api/orgs/{}/sms/policy", a.org()), Some(&a.token)).await;
    assert_eq!(v["send_policy"], "members");
}
...
async fn member_phone_self_or_admin_normalizes_and_clears(db: sqlx::PgPool) {
    ...
    let (st, v) = app.put(&path, Some(&carla.token), json!({"phone": "923 000 111"})).await;
    assert_eq!(st, 200, "{v}");
    ...
    let (st, _) = app.put(&path, Some(&duarte.token), json!({"phone": "923000222"})).await;
    assert_eq!(st, 403);

    // O admin (dono) muda o telefone de outro membro.
    let (st, v) = app.put(&path, Some(&a.token), json!({"phone": "+244 923-000-222"})).await;
    assert_eq!(st, 200, "{v}");
    assert_eq!(v["phone"], "+244923000222");

    // Número inválido: 422, nada muda.
    let (st, v) = app.put(&path, Some(&carla.token), json!({"phone": "123"})).await;
    assert_eq!(st, 422, "{v}");

    // `follow_directory: false` não pede nada — 400.
    let (st, _) = app
        .put(&path, Some(&carla.token), json!({"follow_directory": false}))
        .await;
    assert_eq!(st, 400);

    // `phone: null` limpa a substituição manual.
    let (st, v) = app.put(&path, Some(&carla.token), json!({"phone": null})).await;
    assert_eq!(st, 200, "{v}");
    ...
    let (st, _) = app
        .put(
            &format!("/api/orgs/{}/members/{}/phone", a.org(), uuid::Uuid::new_v4()),
            Some(&a.token),
...
async fn sms_preferences_are_per_account_with_phones_per_active_org(db: sqlx::PgPool) {
    ...
    let (st, v) = app.get("/api/users/me/sms-preferences", Some(&a.token)).await;
    assert_eq!(st, 200, "{v}");
    ...
    let (st, v) = app
        .put("/api/users/me/sms-preferences", Some(&a.token), json!({"contact_opt_out": true}))
        .await;
    assert_eq!(st, 200, "{v}");
    assert_eq!(v["contact_opt_out"], true);
    assert_eq!(v["meeting_opt_out"], false);
    let (_, v) = app.get("/api/users/me/sms-preferences", Some(&a.token)).await;
    assert_eq!(v["contact_opt_out"], true, "persistiu");

    // B não vê as preferências de A nem os telefones de A.
    let (_, v) = app.get("/api/users/me/sms-preferences", Some(&b.token)).await;
    assert_eq!(v["contact_opt_out"], false, "conta diferente, preferência própria");
    assert!(
        v["phones"]
            .as_array()
            .unwrap()
    ...
    let carla = app.add_member(&a, "carla", "member").await;
    let (_, v) = app.get("/api/users/me/sms-preferences", Some(&carla.token)).await;
