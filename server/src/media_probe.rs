...
pub async fn probe_and_store(state: &AppState, rec_id: Uuid, path: &Path) -> MediaInfo {
    ...
    let has_thumb = if info.video_codec.is_some() {
        thumbnail(
    ...
    let r = sqlx::query(
        "UPDATE recordings SET duration_ms = $2, width = $3, height = $4, fps = $5,
                video_codec = $6, audio_codec = $7, has_thumbnail = $8, probed_at = now()
         WHERE id = $1",
    )
    .bind(rec_id)
