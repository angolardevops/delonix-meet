...
pub async fn ws_directo(
    ...
    for b in brutos {
        match b.id {
            Some(id) => {
                match crate::stream_destinations::resolve_for_broadcast(&state, user_id, id).await {
                    Ok(mut d) => {
                        if let Some(r) = b.rotulo.filter(|r| !r.trim().is_empty()) {
                            d.rotulo = r;
                        }
                        destinos.push(d);
                    }
                    Err(m) => return recusa(ws, m),
                }
            }
            None => {
...
async fn emitir(
    ...
    crate::stream_destinations::mark_used(&state, &destinos).await;
...
async fn bombear(
    ...
    crate::stream_destinations::record_outcome(&state, &finais).await;
