...
#[utoipa::path(
    ...
    responses(
        (status = 200, body = MessagePage, description = "Admin: toda a org. Membro: só as mensagens que ele mandou, com o número mascarado."),
        (status = 400, body = crate::openapi::ErrorBody),
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 404, description = "Quem pede não é membro activo da organização.", body = crate::openapi::ErrorBody),
    )
)]
...
#[utoipa::path(
    ...
    responses(
        (status = 200, body = Message, description = "Admin: qualquer mensagem da org. Membro: só a que ele mandou, com o número mascarado."),
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 404, description = "A mensagem não existe nesta org, ou pertence a outra pessoa e quem pede não é admin.", body = crate::openapi::ErrorBody),
    )
)]
...
/// recusado não pode consumir o limite de quem o fez. Reutilizado por
/// `sms_notify` (convites e lembretes de reunião).
...
/// Dois modos: `{to, body}` número escrito, **só admin**; `{user_id, body}`
/// contacto — membro ACTIVO desta org, número resolvido no servidor, sujeito à
/// política da org (`sms_send_policy`) e ao consentimento da pessoa.
///
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, body = crate::openapi::ErrorBody, description = "`to` e `user_id` juntos ou nenhum dos dois, ou corpo vazio."),
        (status = 409, body = crate::openapi::ErrorBody, description = "`Idempotency-Key` já usada por outra pessoa (só membro)."),
        (status = 422, body = crate::openapi::ErrorBody, description = "Sem rota/dispositivo, contacto sem telefone, ou consentimento desligado."),
        (status = 429, body = crate::openapi::ErrorBody),
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 403, description = "Número escrito à mão por quem não é admin, ou contacto sem a política `members`.", body = crate::openapi::ErrorBody),
        (status = 404, body = crate::openapi::ErrorBody),
    )
)]
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 404, description = "Quem pede não é membro activo.", body = crate::openapi::ErrorBody),
    )
)]
...
/// `PUT /api/orgs/{org_id}/sms/policy` — singleton, só admin. Um valor
/// desconhecido é RECUSADO: é uma permissão, e ignorá-lo em silêncio deixava o
/// admin a julgar que a tinha mudado.
#[utoipa::path(
    ...
    responses(
        (status = 200, body = PolicyBody),
        (status = 400, body = crate::openapi::ErrorBody, description = "`send_policy` diferente de `admins`/`members`."),
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 403, description = "Membro sem papel de admin.", body = crate::openapi::ErrorBody),
        (status = 404, body = crate::openapi::ErrorBody),
    )
...
/// `PUT /api/orgs/{org_id}/members/{user_id}/phone` — o próprio ou um admin. A
/// validação é a mesma do envio (`normalize_msisdn`): não se guarda um número
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, body = crate::openapi::ErrorBody, description = "`phone` inválido, ou `follow_directory` com `phone` ao mesmo tempo."),
        (status = 401, body = crate::openapi::ErrorBody),
        (status = 403, description = "Nem admin nem o próprio.", body = crate::openapi::ErrorBody),
        (status = 404, description = "Quem pede não é membro activo, ou o utilizador não é membro activo desta organização.", body = crate::openapi::ErrorBody),
    )
)]
...
pub struct SmsPreferences {
    ...
    /// O telefone do próprio em cada org activa (editável por `PUT
