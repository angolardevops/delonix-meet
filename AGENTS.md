...
- **Organização do backend** (medida a 2026-09-16, [`docs/auditoria-2026-09-16-backend.md`](docs/auditoria-2026-09-16-backend.md)): UM crate, 34 módulos planos, ciclo de 18 módulos, SQL nos handlers, regras copiadas entre BFF e v1 que já divergiram; **sem gRPC nem OpenAPI**. Destino em [ADR-0004](docs/adr/0004-organizacao-alvo-do-backend.md) (**Proposto**): camadas http→service→store, workspace `delonix-meet-*`, uma superfície de API por público, gRPC só máquina-a-máquina (voz/IVR, ai-worker) e nunca browser→servidor. A ordem de migração (§6) não se salta.
...
**Rust**: `AppError` para todos os erros de handler — nunca `unwrap()` em produção. `sqlx::query`/`query_as::<_, T>` — API de runtime, **sem** verificação em compile time (302 chamadas, zero macros): um nome de coluna errado só falha em execução. Migrações `server/migrations/NNNN_*.sql` sequenciais. Novo módulo → declarar em `main.rs` + registar rotas. **Código novo chama a regra, não a copia** (ADR-0004 §5; `scripts/check-arquitectura-catraca.sh` falha se uma contagem subir): pertença → `org::`, auth → extractor existente, saída HTTP → `state.webhook_client`, cripto → a função que existe, `pub(crate)` e nunca `*_pub`, nada de `{"ok": true}`, nada de sessão em `/api/v1`. **Rota nova** → checklist da skill `delonix-meet-api`. **Identificadores novos em inglês**; comentários e docs em português (regra de 2026-09-03).
