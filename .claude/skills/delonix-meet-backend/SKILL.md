...
- **S4:** SSRF no `odoo_url`, no WebDAV e na descoberta OIDC. **S6:** chaves de API sem escopos.
- ~~**S5:** segredos de integração em claro~~ **fechado para webhooks, SSO e WebDAV (R160)**:
  ...
  `reseal_legacy` cifra-o no arranque e de hora a hora. **Resta:** `apikeys.rs` (provisão
  Odoo) grava o `client_secret` em claro — deve chamar `org::seal_sso_client_secret`.
