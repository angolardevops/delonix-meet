...
- **Ficheiros:** `deploy/k8s/51-coturn.yaml` (Deployment+LB), `deploy/k8s/10-config-secret.yaml` (`TURN_HOST`/`FORCE_TURN_RELAY`), `server/src/rooms.rs` (`ice_servers` relay-only), `server/src/sfu.rs` (`RTCConfiguration` relay-only), `server/src/config.rs`. O webrtc-rs TURN client trata o `438 Stale nonce` sozinho (não é bug).
