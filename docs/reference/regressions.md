...
**Ficheiros.** `server/migrations/0052_recording_media_publication.sql`, `0053_recording_chapters_comments_ms.sql`, `0054_recording_captions_views.sql`, `server/src/{recordings,recording_meta,recording_captions,media_probe,recorder,transcription,grpc,org,config,lib,openapi}.rs`, `server/crates/delonix-meet-domain/src/content/{recording,caption,transcription}.rs`, `server/proto/delonix/meet/transcription/v1/transcription.proto`, `server/tests/{recordings_metadata,grpc}.rs`, `ai-worker/{transcriber,job_source,worker}.py` e `ai-worker/tests/test_worker.py` (o worker passa a entregar segmentos e língua), `web/e2e/isolamento.mjs`, `scripts/check-isolamento-cobertura.sh`.
