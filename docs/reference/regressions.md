...
### R183 — Dois servidores com dois modelos de gravação: a UI nova falava com rotas e campos que esta linha não tinha

**Sintoma.** Medido no levantamento UI↔API de 2026-09-17: a UI nova (`integra/validacao-l2`) lê `RecordingLibraryItem` — `duration_ms`, `kind`, `state`, `transcript_status`, contagens, `caption_languages`, `description`, `tags`, `visibility` — e chama `/details`, `/publish`, `/unpublish`, `/thumbnail`, `/views`, `/participants`, `/transcript` e `/captions/*`. Esta linha devolvia `duration_secs`, `category`, `title` e `processing_state`, e nenhuma dessas rotas existia: a biblioteca aparecia sem duração, o leitor sem transcrição nem legendas, e publicar dava `405`. O servidor da UI tinha o modelo, mas com a regra de acesso de antes do S3 (`sql_can_view` sem `archived_at`: um participante ARQUIVADO continuava a ver), recusas em `401`/`403` sem código, comentários apagados FISICAMENTE, o `RECORDINGS_DIR` lido do ambiente por baixo da configuração, e o `ai-worker` a escrever `transcript_segments` directamente no Postgres, por baixo do DLP.

**Regra 1 — o contrato de DADOS é o da UI; a ESTRUTURA é desta linha.** Nomes, unidades e enums vêm da UI; os caminhos seguem as superfícies (singleton `PUT/DELETE …/publication` em vez de verbos `POST /publish`), o envelope com `code`, `core::page`, `201`+`Location`/`204`, e 404-antes-de-403. Migrações só para a frente (0053–0055) convertem os dados: segundos→ms, `lecture`→`training`, `other`→`meeting`, `title`→`filename`, dois capítulos no mesmo segundo afastados 1 ms (a unicidade passa a ser por instante). `title` sai (a UI renomeia pelo `filename`; dois nomes para a mesma coisa divergiam). `status` NÃO ganha `processing`/`transcribing`: o primeiro nunca é observável nesta linha (a linha só nasce depois do ffmpeg), o segundo deriva da reserva da fila — a API deriva os dois. `transcript_error` não se cria: é o `transcription_error` que já existia.

**Regra 2 — publicar é um facto de acesso, não um atalho.** `AccessFacts` ganha `published_to_my_org` (publicada E quem pede é membro ACTIVO de uma org do autor), lido da MESMA junção de pertença; dá ver/reproduzir/comentar, nunca descarregar nem gerir; o arquivado continua a perder tudo. `LIBRARY_VISIBLE_MINE`/`LIBRARY_VISIBLE_PUBLISHED` são `AccessFacts::listed_in` em SQL, e `library_scopes_agree_with_access_facts` compara-os com sete actores (dono, participante, colega, admin, partilhado de fora, arquivado, outra org), privada e publicada, na lista e na página. Controlo: tirar `m.active_member` do SQL faz esse teste e `publication_opens_to_active_org_members_only` falharem.

**Regra 3 — o que a UI não tem não se inventa, e o que o servidor não sabe fica `null`.** A medição (`media_probe`) degrada sem ffprobe: campos NULL, `has_thumbnail=false`, upload e gravador nunca falham por isso. Legendas não publicadas são `404` para quem só vê. Apagar um comentário continua LÓGICO (a UI só precisa que desapareça), e `can_delete` = autor ou quem gere (moderação, auditada como `recording.comment_moderated`). `captions/generate` e `chapters/generate` ficam por portar, sem rota nem stub.

**Portão.** `server/tests/recordings_metadata.rs` (17, Postgres real, incluindo a conversão das migrações a partir de dados na forma antiga e um webm real medido com ffmpeg quando existe), `tests/grpc.rs` (segmentos com DLP), tabelas de verdade em `domain::content::{recording,caption,transcription}`, `buf breaking` contra a base, (os campos novos do `CompleteJobRequest` são compatíveis no fio, mas partem quem constrói a mensagem em Rust com um literal: `tests/{grpc,notifications}.rs` passam a `..Default::default()`), e `web/e2e/isolamento.mjs` com as rotas novas (o `check-isolamento-cobertura.sh` passa a aceitar um parâmetro a meio do caminho, `/captions/{lang}/vtt`).

**Ficheiros.** `server/migrations/0053_recording_media_publication.sql`, `0054_recording_chapters_comments_ms.sql`, `0055_recording_captions_views.sql`, `server/src/{recordings,recording_meta,recording_captions,media_probe,recorder,transcription,grpc,org,config,lib,openapi}.rs`, `server/crates/delonix-meet-domain/src/content/{recording,caption,transcription}.rs`, `server/proto/delonix/meet/transcription/v1/transcription.proto`, `server/tests/{recordings_metadata,grpc}.rs`, `ai-worker/{transcriber,job_source,worker}.py` e `ai-worker/tests/test_worker.py` (o worker passa a entregar segmentos e língua), `web/e2e/isolamento.mjs`, `scripts/check-isolamento-cobertura.sh`.

### R184 — Dois servidores divergiram nas reuniões agendadas: a UI escrevia opções que esta linha não tinha

**Sintoma.** A UI nova (ramo `integra/validacao-l2`, servidor próprio) agenda uma reunião com `format` (`meeting|training|broadcast|hybrid`), `waiting_room`, `auto_record` e `record_quality` (`2160p|1080p|720p|audio`), lê `invitee_count` e `external_source` na lista, e responde `tentative` ao convite. Contra o servidor desta linha: o serde deitava fora as quatro opções sem erro (201 sem as ter gravado), a lista não trazia os dois campos, `tentative` dava `400`, o `POST /api/meetings/{meeting_id}/start` criava SEMPRE uma sala `normal` sem sala de espera, e a v1 criava a sala `normal` com o `waiting_room` do pedido e mais nada. A «gravação automática» agendada nunca gravava.

**Causa raiz.** As duas linhas de servidor evoluíram em paralelo: a da UI fez a migração 0046 e mexeu em `meetings`/`rooms`/`recorder`/`signaling` (commit `5c22661`); esta linha reorganizou rotas e superfícies. A decisão foi adoptar o contrato de DADOS da UI sobre as rotas e a estrutura desta linha.
...
**Adaptações nesta linha.** A 0046 da UI passa a 0056. O «só orador» mede a altura pelo cabeçalho IVF (`ivf_dims`) em vez do `media_probe`, que esta linha não tem. O estado `processing` e o progresso da composição da UI NÃO foram portados.
...
**Ficheiros.** `server/migrations/0056_meeting_session_options.sql`, `server/crates/delonix-meet-domain/src/conferencing/session_options.rs`, `server/src/{meetings,meetings_v1,rooms,recorder,signaling,lib}.rs`, `server/tests/meeting_session_options.rs`.
