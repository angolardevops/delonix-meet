...
**Ficheiros.** `server/crates/delonix-meet-core/src/egress.rs`, `server/src/net_guard.rs`, `server/src/{auth,odoo,odoo_sso,org,storage,webhooks,ai,lib,config}.rs`, `server/tests/egress_guard.rs` (o login Odoo NÃO abre ligação a `127.0.0.1` sem allowlist e abre com ela; recusas ao gravar com controlo positivo).
