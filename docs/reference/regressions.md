...
**Correcção.** `vendor/webrtc` (0.17.1 intacto num commit, a alteração no seguinte) + `[patch.crates-io]`: um SSRC que o SDP remoto declara não passa pela sonda — o stream fica para o receiver, sem ser lido nem fechado. E a bomba de RTP do `sfu.rs` passa a registar **porque** terminou (`sfu track terminou … error=`), para que «track unpublished» deixe de ser indistinguível de «o publicador saiu».
