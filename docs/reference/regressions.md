...
### R60 — Conflito de merge DENTRO de um comentário, resolvido em código válido para dois portões e inválido para o terceiro
- **Sintoma:** `tsc --noEmit` verde, 156 testes verdes, e o `vite build` a rebentar com `Error: expected selector` — num ficheiro que tinha acabado de ser dado por resolvido.
- **Causa raiz:** o `<<<<<<< HEAD` caiu imediatamente a seguir a `/* ============`, uma linha que os DOIS lados tinham. Ao concatenar os dois lados, o `/*` de abertura foi consumido pelo bloco de cima e o bloco de baixo ficou com o corpo do comentário **sem abertura** — texto solto onde o Sass esperava um seletor. Cada metade parecia íntegra na revisão.
- **Regra:** **um conflito cujo primeiro `<<<<<<<` está logo a seguir a uma linha partilhada não se resolve por concatenação** — a linha partilhada pertence a um dos lados e desaparece do outro. Confirma-se lendo as fronteiras, não o meio.
