...
import { listBranches, listEmployees, listGroups, listMeetingRooms, orgStats } from '../api'
      ...
      <AsyncSection state={list.state} onRetry={list.reload}>
        {() =>
          !org ? (
            ...
            <AdminBody key={org.id} org={org} onOrgChanged={list.reload} />
          )
        }
      </AsyncSection>
      {creatingOrg && (
...
function AdminBody({ org, onOrgChanged }: { org: OrgSummary; onOrgChanged: () => void }) {
  const { t } = useTranslation()
  const { user } = useShell()
