...
import { Alert, Avatar, Button, Empty, Select, Spinner, StatusBadge, Toggle, cx } from '../ui/kit'
...
export default function Lobby({ code }: { code: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        signal.on('peer-role', (m) => setPeers((p) => p.map((x) => (x.peer_id === m.peer_id ? { ...x, can_admit: m.can_admit } : x))))
        signal.on('server-recording', (m) => setRecServidor(m.active ? m.by : null))
  ...
  const breakouts: BreakoutsApi = {
    ...
    minutes,
    setMinutes,
    create: (count) => send({ type: 'breakouts-create', count, minutes: minutes || null }),
  ...
  const linhas = useMemo<Linha[]>(() => {
    const out: Linha[] = []
    if (filtro === 'todos' && connected) out.push({ kind: 'eu', nome: me?.username ?? '' })
  ...
  const papel = (p: PeerInfo) =>
    p.host ? (
      ...
      <span className="lb-role is-cohost" title={t('room.papel.coAnfitriaoDica')}>
  ...
  const origem = (p: PeerInfo) =>
    p.is_pstn ? t('room.papel.telefone') : p.is_bot ? t('room.papel.assistente') : `${p.mic ? t('room.lobby.micLigado') : t('room.lobby.micDesligado')} · ${p.cam ? t('room.lobby.camLigada') : t('room.lobby.camDesligada')}`
        ...
        <div className="lb-grid">
          <section className="lb-main" aria-labelledby="lb-part-h">
            <div className="lb-main__head">
              ...
              <span className="dx-num dx-muted">
                {t('room.lobby.resumo', { naSala: peers.length + (connected ? 1 : 0), espera: waiting.length })}
                {emParalelas > 0 && ` · ${t('room.lobby.emParalelas', { count: emParalelas })}`}
              </span>
              <span className="dx-spacer" />
                            ...
                            <td>
                              <span className="lb-person">
                                ...
                                <span>
                                  ...
                                  <small className="dx-num dx-muted">{t('room.lobby.naConsola')}</small>
                                </span>
                              </span>
                            </td>
                            <td>
                        ...
                        return (
                          <tr key={`bo-${l.sala.code}-${l.nome}`}>
                            ...
                            <td className="lb-actions">
                              <Select
                                aria-label={t('room.paralelas.moverPessoa', { nome: l.nome })}
                                value={l.sala.code}
                                onChange={(e) => breakouts.moveUser(l.nome, e.target.value)}
                              >
                                {rooms.map((o) => (
                        ...
                        return (
                          <tr key={`w-${p.peer_id}`}>
                            <td>
                              <span className="lb-person">
                                ...
                                <span>
                                  ...
                                  <small className="dx-num dx-muted">{p.is_pstn ? t('room.papel.telefone') : t('room.avisos.querEntrar')}</small>
                                </span>
                              </span>
                            </td>
                            <td>
                            ...
                            <td className="lb-actions">
                              ...
                              <button type="button" className="lb-act" onClick={() => admit(p.peer_id, false)}>
                                {t('room.avisos.negar')}
                              </button>
                            </td>
                          </tr>
                        ...
                        <tr key={p.peer_id}>
                          <td>
                            <span className="lb-person">
                              ...
                              <span>
                                <strong>{p.username}</strong>
                                <small className="dx-num dx-muted">{origem(p)}</small>
                              </span>
                              {p.hand && (
                            ...
                            {!p.host && (
                              <>
                                ...
                                <button
                                  ...
                                  onClick={() => send({ type: 'promote-admit', to: p.peer_id, allowed: !p.can_admit })}
                                >
                                  {p.can_admit ? t('room.lobby.retirarCoAnfitriao') : t('room.lobby.tornarCoAnfitriao')}
                                </button>
          ...
          <div className="lb-side">
            <section className={cx('lb-card lb-waitcard', waiting.length > 0 && 'is-busy')} aria-labelledby="lb-wait-h">
                ...
                waiting.map((w) => (
                  <div key={w.peer_id} className="lb-queue__row">
                    ...
                    <span className="lb-queue__who">
                      ...
                      <small className="dx-num dx-muted">{w.is_pstn ? t('room.papel.telefone') : t('room.avisos.querEntrar')}</small>
                    </span>
              ...
              {waiting.length > 1 && (
                <Button block variant="outline" onClick={() => waiting.forEach((w) => admit(w.peer_id, true))}>
