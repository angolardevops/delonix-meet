 ...
 * aprovação) vive no diálogo largo — ver RolesDialog.tsx. O servidor
 * (ADR-0008) decide tudo; aqui só se mostra e se pede.
 */
import { useState } from 'react'
import { useTranslation } from 'react-i18next'
import { listApprovalRequests, listOrgRoles } from '../../api'
...
export default function RolesCard({ orgId }: { orgId: string }) {
  ...
  const pending = requests.state.s === 'ready' ? requests.state.d.items.length : 0
        ...
        {(r) => (
          <div className="org-card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <p className="dx-muted" style={{ margin: 0 }}>
              {t('rbac.resumo', { papeis: r.items.length })}
            </p>
