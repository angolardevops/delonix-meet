...
/** Singleton: o link público cria-se com `PUT` (ADR-0004 §4, regra 4 do mapa de rotas). */
...
/** A resposta ao convite é o MEU registo de convidado — singleton, logo `PUT`. */
...
/**
 * A quarentena passou a viver debaixo da organização: o `org_id` é PARTE DO
 * CAMINHO e deixou de ser opcional. Não há mais rota para «todas as
 * organizações que administro» — o servidor lê uma só org por pedido.
 */
...
/** A acta é um singleton da sala: escreve-se com `PUT` e lê-se com `roomNotes`. */
...
/** Superfície do OPERADOR (`/api/operator/v1`), já não a do inquilino. */
...
export const ackMissedCalls = () =>
  request('/api/users/me/missed-calls/acknowledge', { method: 'POST' })
...
/** Leitura da acta da sala: o MESMO caminho que o `saveMinutesByRoom` escreve, com `GET`. */
...
export async function recordingObjectUrl(rec: Recording): Promise<string> {
  // O FICHEIRO vive em `/content`; o caminho nu devolve os metadados.
...
/**
 * A acção deixou de ter caminho de topo: vive debaixo do plano, que vive
 * debaixo da reunião. Quem chama tem de trazer o `meetingId` — não há forma de
 * o derivar do `itemId` no cliente.
 */
...
/**
 * As salas de voz passaram a viver debaixo da organização
 * (`/api/orgs/{org_id}/voice/rooms…`): o `orgId` é agora parte do caminho e
 * primeiro argumento destas quatro funções.
 */
