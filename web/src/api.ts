...
/**
 * Quarentena da organização. O `org_id` deixou de ser um parâmetro de query
 * opcional e passou a ser do CAMINHO
 * (`GET /api/orgs/{org_id}/analytics/quarantine`): com ele obrigatório no
 * servidor, a variante «todas as organizações que administro» deixou de existir
 * — quem chama escolhe UMA organização.
 */
...
/**
 * O item do plano passou a viver DEBAIXO da reunião
 * (`/api/meetings/{meeting_id}/action-plan/items/{item_id}`): o servidor exige
 * agora que o item seja mesmo dessa reunião, por isso o `meetingId` deixou de
 * ser dispensável do lado do cliente.
 */
...
// (`/api/orgs/{org_id}/voice/rooms…`): o `orgId` é do caminho, e o servidor
// recusa com 404 a sala que não seja dessa organização.
...
/** Encerrar responde `204` sem corpo — por isso não passa pelo `request`. */
