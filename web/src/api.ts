...
/**
 * Quarentena por organização. A rota passou a viver DEBAIXO da organização
 * (`/api/orgs/{org_id}/analytics/quarantine`), por isso o `org_id` deixou de ser
 * um filtro opcional na query e passou a ser parte do caminho: não há mais rota
 * para «todas as organizações de uma vez».
 */
...
/**
 * O item do plano passou a viver debaixo da reunião: o servidor exige o
 * `meeting_id` no caminho e confirma que o item é MESMO dela.
 */
...
/** `204` sem corpo — por isso `requestEmpty`, e não `request` (que lê sempre JSON). */
...
// As salas de voz passaram a viver debaixo da organização: o `org_id` é parte
// do caminho, não se deriva no servidor. Quem chama tem de saber de que
// organização é a sala — como já acontece nos DIDs e no CDR aqui ao lado.
...
/** `204` sem corpo desde a reorganização das rotas — daí o `requestEmpty`. */
