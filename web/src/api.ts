...
/**
 * Quarentena POR ORGANIZAÇÃO. O `org_id` deixou de ser query opcional e passou a
 * ser segmento obrigatório do caminho — com ele caiu o modo «todas as minhas
 * organizações», que o servidor já não serve (`quarantine_analytics` exige
 * `Path(org_id)` e `require_admin`).
 */
...
/** Leitura da acta (era `GET …/notes`): mesmo caminho do `saveMinutesByRoom`, `GET` em vez de `PUT`. */
...
export async function recordingObjectUrl(rec: Recording): Promise<string> {
  // O ficheiro vive em `/content`; o `GET` sem sufixo devolve agora os METADADOS.
...
/** Apagar responde `204` sem corpo — por isso `requestEmpty`, não `request`. */
 ...
 * (`/api/orgs/{org_id}/voice/rooms…`): o `orgId` deixou de ser derivado da
 * sessão e é agora segmento do caminho.
 */
...
/** Encerrar responde `204` sem corpo (era `{ ok: true }`). */
