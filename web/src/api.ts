...
/** O link público é um singleton do recurso → `PUT` (6a854af). */
...
/** A minha resposta ao convite é um sub-recurso singleton → `PUT` (6a854af). */
...
/**
 * Quarentena POR ORGANIZAÇÃO. O `org_id` deixou de ser um parâmetro de query
 * opcional e passou a estar no caminho (`/api/orgs/{org_id}/analytics/
 * quarantine`, 6a854af) — com isso desapareceu o âmbito «todas as minhas
 * organizações», que era o que a ausência do `org_id` pedia ao servidor.
 */
...
export const saveMinutesByRoom = (code: string, minutes: string, transcript: string) =>
  // A acta da sala é um singleton → `PUT` (6a854af). O GET do mesmo caminho é
  // o `roomNotes` — a leitura e a escrita partilham a rota, não a função.
...
/** Ligar/desligar o link público — singleton, por isso `PUT` (6a854af). */
...
export const updateOrgSettings = (
  ...
  // Alteração parcial da organização → `PATCH /api/orgs/{org_id}` (6a854af).
...
/** Leitura da acta da sala: `GET` do mesmo caminho que o `saveMinutesByRoom` escreve. */
...
/**
 * A acção vive debaixo do plano, que vive debaixo da reunião: desde 6a854af o
 * caminho é `/api/meetings/{meeting_id}/action-plan/items/{item_id}` e o
 * `meeting_id` deixou de ser opcional.
 */
...
// As salas de voz passaram a viver debaixo da organização (6a854af): o
// `org_id` é agora parte do caminho, não uma dedução do lado do servidor.
