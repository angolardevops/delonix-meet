...
# O ADR-0005 refina a lista (tabela `| \`delonix-meet-x\` |`) — os crates dele também não são revisores.
crates |= set(re.findall(r'\| `(delonix-meet-[a-z]+)` \|', open('docs/adr/0005-backend-enterprise-contextos-edicoes-e-entrega.md', encoding='utf-8').read()))
